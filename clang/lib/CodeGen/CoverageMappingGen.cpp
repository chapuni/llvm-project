...
namespace {
using MCDC_Cond_ID = CounterMappingRegion::MCDC_Cond_ID;
...
private:
  ...
  std::vector<MCDC_Cond_ID> AndRHS;
  std::vector<MCDC_Cond_ID> OrRHS;
  std::vector<const BinaryOperator *> NestLevel;
  llvm::DenseMap<const Stmt *, MCDC_Cond_ID> &CondIDs;
  llvm::DenseMap<const Stmt *, unsigned> &MCDCBitmapMap;
  MCDC_Cond_ID NextID = 1;
  ...
  void pushRHS(const BinaryOperator *E) {
    std::vector<MCDC_Cond_ID> &rhs = isLAnd(E) ? AndRHS : OrRHS;
...
struct CounterCoverageMappingBuilder
  ...
  size_t pushRegion(Counter Count,
                    ...
                    MCDC_Cond_ID ID = 0, MCDC_Cond_ID TrueID = 0,
                    MCDC_Cond_ID FalseID = 0) {
    ...
    RegionStack.emplace_back(Count, FalseCount,
                             MCDCParameters(ID, TrueID, FalseID), StartLoc,
                             EndLoc);
        ...
        if (StartLocs.insert(Loc).second) {
          if (I.isBranch())
            SourceRegions.emplace_back(
                ...
                MCDCParameters(I.getMCDCParams().ID, I.getMCDCParams().TrueID,
  ...
  CounterCoverageMappingBuilder(
      ...
      llvm::DenseMap<const Stmt *, unsigned> &CounterMap,
      llvm::DenseMap<const Stmt *, unsigned> &MCDCBitmapMap,
      llvm::DenseMap<const Stmt *, MCDC_Cond_ID> &CondIDMap, SourceManager &SM,
      const LangOptions &LangOpts)
