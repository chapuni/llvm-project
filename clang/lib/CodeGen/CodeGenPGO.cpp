        ...
        if (LogOpStack.empty()) {
          ...
          if (SplitNestedLogicalOp) {
            unsigned DiagID = Diag.getCustomDiagID(
                ...
                "MC/DC: Unsupported boolean expression; "
            ...
            unsigned SizeInBits = llvm::alignTo(1L << NumCond, CHAR_BIT);
...
void CodeGenPGO::mapRegionCounters(const Decl *D) {
  ...
  // If MC/DC is enabled, set the MaxConditions to the preset value. Otherwise,
  // set it to zero. Currently, the max value is hard-coded at 6, but in the
  // future, this could be adjusted by way of a command-line option.
...
void CodeGenPGO::emitMCDCTestVectorBitmapUpdate(CGBuilderTy &Builder,
  ...
  auto expr_mcdc_bitmap_map_iterator = RegionMCDCBitmapMap->find(S);
  if (expr_mcdc_bitmap_map_iterator == RegionMCDCBitmapMap->end())
    return;

  // Extract the ID of the global bitmap associated with this expression.
  unsigned MCDCTestVectorBitmapID = expr_mcdc_bitmap_map_iterator->second;
  auto *I8PtrTy = llvm::Type::getInt8PtrTy(CGM.getLLVMContext());
...
void CodeGenPGO::emitMCDCCondBitmapUpdate(CGBuilderTy &Builder, const Expr *S,
  ...
  auto expr_mcdc_cond_id_map_iterator = RegionCondIDMap->find(S);
  if (expr_mcdc_cond_id_map_iterator == RegionCondIDMap->end())
    return;

  // Extract the ID of the condition we are setting in the bitmap.
  unsigned CondID = expr_mcdc_cond_id_map_iterator->second;
